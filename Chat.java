			...
			while (users.hasNext()){
				...
				if (!users.hasNext()){
					for (final ConversationContext conversation : user.conversations()) {
						System.out.format("\nCONVERSATION: \"%s\" Created at: %s || UUID: %s CreatorUUID: %s\n",
							conversation.conversation.title, conversation.conversation.creation.HMtime(), 
						...
						for (MessageContext message = conversation.firstMessage(); message != null; message = message.next()) {
							System.out.format("\tMESSAGE %s : \"%s\" || UUID: %s AuthorUUID: %s\n", 
								message.message.creation.HMtime(), message.message.content, 
								message.message.id, message.message.author);			 
						}	
					}
				}
			}
